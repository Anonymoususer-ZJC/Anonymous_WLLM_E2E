...
class GPTParaphraser(TextEditor):
    ...
    def edit(self, text: str, reference=None):
        ...
        openai_util = OpenAIAPI(model=self.openai_model, temperature=0.2,
                                system_content="Your are a helpful assistant to rewrite the text.")
...
class TextSynonymReplacer:
    def __init__(self, model_path='bert-large-uncased', target_prob=0.2):
        """
        初始化TextSynonymReplacer类。

        参数:
        - model_path: BERT模型的路径，默认为'bert-large'。
        - target_prob: 目标替换概率，默认为0.2。
        """
    ...
    def edit(self, text: str, reference=None):
        ...
        # 计算要替换的单词数量
        num_to_replace = int(min(self.target_prob, len(replaceable_indices) / num_words) * num_words)
        indices_to_replace = random.sample(replaceable_indices, num_to_replace)
        ...
        print(f"Target Replace Rate: {self.target_prob}, Real Replace Rate: {real_replace / num_words}")
        return ' '.join(words)

    def get_wordnet_synonyms(self, word):
        """
        从WordNet获取一个单词的所有同义词。
...
class EmojiAttacker(TextEditor):
    """使用指定的模型进行文本释义."""

    def __init__(self, model_name_or_path: str, prompt: str) -> None:
        """
        Initialize the EmojiAttacker with the specified model and prompt.

        Parameters:
            model_name_or_path (str): The path or name of the model to use.
            prompt (str): The prompt to use for paraphrasing.
        """
    ...
    def edit(self, text: str, reference=None):
        """Paraphrase the text using the specified model."""
        # 将prompt和text结合起来，形成输入
        input_text = self.prompt + text
